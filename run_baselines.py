...
class BaselineGCN(torch.nn.Module):
    """
    Standard GCN Baseline for MSP (Maximum Softmax Probability) OOD Detection.
    """
...
def main():
    ...
    # 2. Train Standard GCN
    model = BaselineGCN(dataset.num_features, 64, len(id_classes)).to(device)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        # Train only on ID nodes using standard CrossEntropy
        loss = F.cross_entropy(out[train_mask], data.y[train_mask])
        loss.backward()
        optimizer.step()

    # 3. Inference using MSP (Maximum Softmax Probability)
    model.eval()
    with torch.no_grad():
        ...
        # MSP Score: 1.0 - max probability (higher score means more likely OOD)
    ...
    print(f"Baseline: Standard GCN (MSP)")
